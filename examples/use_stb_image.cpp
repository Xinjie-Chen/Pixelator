...
int main(int argc, char **argv) {
  ...
  int query_row = 3;
  int query_col = 2;
  const auto GetIndex = [&](int query_row, int query_col) {
    return channels * (query_row * cols + query_col);
  };
  const auto Output = [&](int query_row, int query_col, const Color color) {
  ...
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      const Color color = {image_data[GetIndex(i, j)],
                           image_data[GetIndex(i, j) + 1],
                           image_data[GetIndex(i, j) + 2]};
      Output(i, j, color);
    };
  }
